...
The `cdk` directory contains all the necessary components including Bedrock AgentCore and related resources. There are direct references between `cdk` and the runtime `src/`. For example, the IaC code expects to find the
...
There is also a `.bedrock_agentcore.yaml` file output in the `recipeRecommendationAgent` directory. This file contains a minimal definition with your agent name when it is created. After deploying the project, when running
